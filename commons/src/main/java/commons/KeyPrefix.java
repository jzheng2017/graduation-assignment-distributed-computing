package commons;

/**
 * All key prefixes used are defined here
 */
public class KeyPrefix {
    public static final String WORKER_REGISTRATION = "worker-registration";
    public static final String WORKER_STATISTICS = "worker-statistics";
    public static final String WORKER_HEARTBEAT = "worker-heartbeat";
    public static final String CONSUMER_CONFIGURATION = "consumer-configuration";
    public static final String CONSUMER_STATUS = "consumer-status";
    public static final String CONSUMER_TOPIC_OFFSET = "consumer-topic-offset";
    public static final String PARTITION_COUNT = "partitions";
    public static final String PARTITION_ASSIGNMENT = "partition-assignment";
    public static final String PARTITION_CONSUMER_ASSIGNMENT = "partition-consumer-assignment";

    private KeyPrefix() {
        //no instantiation
    }
}
